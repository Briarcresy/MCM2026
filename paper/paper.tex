...
\subsection{Scenario c: Hybrid Transportation Strategy (Multi-Objective Mixed Integer Linear Programming Model)}
...
Minimizing total cost typically favors slower but cheaper transportation strategies, whereas minimizing completion time encourages faster but more expensive delivery methods.
These conflicting goals motivate a multi-objective optimization framework.

To solve the problem, the weighted-sum method is adopted:

\begin{equation}
    \min F = \alpha \frac{C}{C^{\max}} + (1-\alpha)\frac{T}{T^{\max}}, \quad \alpha \in [0,1]
\end{equation}

where $C^{\max}$ and $T^{\max}$ are normalization constants used to avoid scale dominance.
By varying the weight parameter $\alpha$, different trade-offs between cost efficiency and construction speed can be explored.

\subsubsection{Model Interpretation}

The proposed multi-objective mixed integer linear programming model provides a structured framework for determining the optimal annual allocation of material transportation between space elevators and rockets.
The binary variables explicitly capture the project timeline, while the continuous variables control transportation volumes, enabling transparent evaluation of cost--time trade-offs.

...
To determine a recommended hybrid strategy, the weight parameter $\alpha$ is varied continuously over a prescribed range.
Figure~\ref{fig:k_alpha_curve} shows the variation of the minimum composite objective value $F$ as a function of $\alpha$.
The curve exhibits a clear minimum, indicating an optimal balance between construction time and transportation cost.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{k_vs_alpha.pdf}
    \caption{Variation of the weighted objective value with respect to $\alpha$.}
    \label{fig:k_alpha_curve}
\end{figure}

The corresponding optimal solution is highlighted on the Pareto frontier in Figure~\ref{fig:optimal_on_pareto}.
This solution represents the best compromise under balanced priorities and is selected as the recommended hybrid strategy.

...
To evaluate the feasibility of constructing a 100,000-person Moon Colony, we develop a mathematical model to analyze large-scale material transportation from Earth to the Moon. The model compares three transportation strategies: the Space Elevator System, traditional rocket launches, and a hybrid combination of both.

The proposed framework focuses on two key metrics—total transportation time and overall cost—to assess the efficiency and practicality of each strategy. By formulating unified expressions under consistent assumptions, the model enables a fair comparison among different transportation schemes.

In addition, the model is designed to accommodate non-ideal operating conditions and extended supply requirements, providing a foundation for robustness analysis and long-term sustainability evaluation.
...
\[
C_0 = 44{,}940.9 \text{ million USD}.
\]
Considering diminishing learning effects over time, a conservative learning rate of $\beta = 3.7139\%$ is adopted.
...
Substituting the parameters into the cost model, the estimated transportation cost for a 150-ton payload in the year 2050 is approximately 73 million USD per launch. For modeling simplicity and to account for uncertainty, this value is rounded to 73 million USD per mission.

In addition to cost, the time efficiency of rocket transportation is characterized by the launch interval of a single launch site. Based on historical data from Cape Canaveral Spaceport, the initial launch interval is estimated as $T_0 = 13.27$ days. A similar learning-based model is applied:
\begin{equation}
...
\subsection{Scenario III: Hybrid Transportation Strategy}

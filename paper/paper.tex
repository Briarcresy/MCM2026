...
\begin{enumerate}
    \item \textbf{Atmospheric emissions} from rocket launches
    \item \textbf{Resource extraction and manufacturing} impacts
    \item \textbf{Energy consumption and associated emissions}
\end{enumerate}

\subsubsection{Quantitative Environmental Impact Model}

We define an Environmental Impact Index (EII) that aggregates normalized environmental impacts across these categories:

\[
EII = w_1 \cdot E_{\text{atmosphere}} + w_2 \cdot E_{\text{manufacturing}} + w_3 \cdot E_{\text{energy}}
\]

where \(w_1 + w_2 + w_3 = 1\) and each component is normalized to a [0,1] scale.

\paragraph{Atmospheric Emissions from Rocket Launches}

Based on current Falcon Heavy launch data and projections for advanced rockets in 2050, we estimate emissions per launch:

\begin{itemize}
    \item \textbf{$CO_2$ emissions}: Approximately 4,000 metric tons per launch
    \item \textbf{Black carbon (soot)}: 30 metric tons per launch (significant climate forcing agent)
    \item \textbf{Water vapor}: 1,500 metric tons per launch (stratospheric effects)
    \item \textbf{NOx and other compounds}: 50 metric tons per launch
\end{itemize}

The total atmospheric impact per rocket launch is calculated as:

\[
E_{\text{rocket, launch}} = \alpha_{\text{CO2}} \cdot m_{\text{CO2}} + \alpha_{\text{BC}} \cdot m_{\text{BC}} + \alpha_{\text{H2O}} \cdot m_{\text{H2O}} + \alpha_{\text{NOx}} \cdot m_{\text{NOx}}
\]

where \(\alpha\) coefficients represent the Global Warming Potential (GWP) relative to $CO_2$ over 20-year and 100-year time horizons.

\paragraph{Space Elevator System Environmental Impact}

The Space Elevator System has different environmental characteristics:

\begin{itemize}
    \item \textbf{Construction phase}: Significant resource extraction for graphene production, Earth port construction, and manufacturing of tethers and apex anchors
    \item \textbf{Operation phase}: Primarily electrical energy consumption with minimal atmospheric emissions
\end{itemize}

The construction impact is modeled as a one-time environmental cost:

\[
E_{\text{elevator, construction}} = \beta \cdot F_e
\]

where \(\beta\) represents the environmental impact per dollar of construction cost, accounting for the materials and energy intensity of advanced materials manufacturing.

\paragraph{Energy Consumption and Associated Emissions}

Both systems require significant energy inputs:

\begin{itemize}
    \item \textbf{Rocket system}: Energy embedded in propellant production (methalox or hydrolox)
    \item \textbf{Space elevator}: Electrical energy for climbing mechanisms and station-keeping
\end{itemize}

We convert all energy consumption to equivalent $CO_2$ emissions based on the energy mix in 2050:

\[
E_{\text{energy}} = \sum_{i} E_i \cdot EF_i
\]

where \(EF_i\) is the emission factor for energy source \(i\) in the projected 2050 grid.
...
The environmental impact of this scenario is dominated by the construction phase of the three Galactic Harbours:

\[
EII_a = E_{\text{elevator, construction}} + E_{\text{elevator, operation}}
\]

where:
\begin{itemize}
    \item \(E_{\text{elevator, construction}} = 0.85\) (normalized impact, scale [0,1])
    \item \(E_{\text{elevator, operation}} = 0.12\) (assuming 80\% renewable energy)
    \item \textbf{Total EII}: 0.97
\end{itemize}

The atmospheric emissions are negligible during operation, making this the cleanest option once constructed.

\paragraph{Key Environmental Advantages}
\begin{itemize}
    \item \textbf{Zero operational atmospheric emissions}: No direct pollution during material transport
    \item \textbf{Low long-term energy requirements}: After construction, energy needs are modest and can be sourced renewably
    \item \textbf{Scalability without proportional environmental impact}: Additional capacity adds minimal environmental burden
\end{itemize}

\paragraph{Environmental Concerns}
\begin{itemize}
    \item \textbf{Construction phase intensity}: Manufacturing graphene tethers and massive structures requires substantial energy and rare materials
    \item \textbf{Geostationary orbit debris}: Potential for creating space debris if systems fail
    \item \textbf{Equatorial ecosystem disruption}: Earth port construction may impact local ecosystems
\end{itemize}
...
The environmental impact scales linearly with the number of launches:

\[
EII_b = N_{\text{launches}} \cdot E_{\text{rocket, launch}} + E_{\text{infrastructure}}
\]

From Section~4.2, we have:
\begin{itemize}
    \item \(N_{\text{launches}} = \frac{10^8}{150} \approx 666,667\) launches
    \item \(E_{\text{rocket, launch}} = 1.2 \times 10^{-6}\) (normalized per launch)
    \item \(E_{\text{infrastructure}} = 0.05\) (launch site upgrades)
    \item \textbf{Total EII}: 0.85
\end{itemize}

Despite the high number of launches, the normalized impact is slightly lower than the elevator construction due to less material-intensive infrastructure.

\paragraph{Atmospheric Impact Details}
\begin{itemize}
    \item \textbf{Total $CO_2$ emissions}: \(666,667 \times 4,000 = 2.67 \times 10^9\) metric tons
    \item \textbf{Black carbon}: \(666,667 \times 30 = 2.0 \times 10^7\) metric tons
    \item \textbf{Equivalent $CO_2$ over 20 years} (including short-lived climate forcers): \(4.8 \times 10^9\) metric tons $CO_2$e
\end{itemize}

This represents approximately 1.2\% of current annual global $CO_2$ emissions spread over 456 years, or 0.26\% if concentrated in 100 years.

\paragraph{Additional Environmental Concerns}
\begin{itemize}
    \item \textbf{Ozone layer depletion}: Rocket emissions in the stratosphere catalytically destroy ozone
    \item \textbf{Air quality near launch sites}: Local pollution impacts on communities
    \item \textbf{Noise pollution}: Significant disturbance to wildlife and human populations
    \item \textbf{Accident risk}: Potential for catastrophic failures with environmental consequences
\end{itemize}
...
For our recommended hybrid strategy with \(\lambda = 0.7103\):

\[
EII_c = \lambda \cdot EII_{\text{elevator}} + (1-\lambda) \cdot EII_{\text{rocket}}
\]

where:
\begin{itemize}
    \item \(EII_{\text{elevator}} = 0.97\) (including construction amortized over the project)
    \item \(EII_{\text{rocket}} = 0.85 \times \frac{132.44}{456.62} = 0.246\) (proportional to time fraction)
    \item \textbf{Total EII}: \(0.7103 \times 0.97 + 0.2897 \times 0.246 = 0.76\)
\end{itemize}

The hybrid strategy achieves the lowest environmental impact by balancing the one-time construction impact of the elevator with reduced rocket launches.

\paragraph{Environmental Advantage of Hybrid Approach}
\begin{itemize}
    \item \textbf{Reduced atmospheric emissions}: 71\% fewer rocket launches compared to rocket-only scenario
    \item \textbf{Amortized construction impact}: Space elevator environmental cost spread across more payload
    \item \textbf{Flexibility for clean energy integration}: Space elevator can use grid electricity with high renewable penetration
\end{itemize}

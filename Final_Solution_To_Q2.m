...
% ----------------------------------------------------------------------------
% 泊松分布（电梯年故障次数）
lambda_elevator = 0.02;        

% 混合威布尔分布参数
% 太空电梯故障延误分布（已优化）
elevator_weibull_params = struct(...
    'beta1', 2.2, ...          % 形状参数1：从2.0调整到1.8
    'theta1', 30/365, ...     % 尺度参数1：从180/365调整到150/365
    'beta2', 1.5, ...          % 形状参数2：从1.5调整到1.3
    'theta2', 8, ...          % 尺度参数2：从25调整到20
    'alpha', 0.001 ...          % 混合比例：从0.1调整到0.08
);
...
function [time_add, cost_add, weight_add, fault_occurred] = ...
    ...
    % 使用输入参数替代全局变量
    local_t_R = 2.5/365;  % 火箭单次时间：2.5天 = 2.5/365 年
    local_c_R = 37000;    % 火箭单次总成本：10×3700 = 37000 万美元
    local_w_R = 1500;     % 火箭单次总运力：10×150 = 1500 吨
    ...
    if rand() < fault_prob
        ...
        time_add = local_t_R * batch_size;
        cost_add = local_c_R * batch_size;
        if batch_size == 10
            weight_add = local_w_R;
        else
            weight_add = local_w_R * batch_size;
        end
    end
end
...
for sim_idx = 1:num_simulations
    ...
    while (W0 - W_total) >= remaining_threshold
        ...
        if rand() < rocket_fault_prob
            ...
            T_R = T_R + local_t_R * 10;
            C_R = C_R + local_c_R * 10;
            W_R = W_R + local_w_R;
        end
    ...
    if remaining_weight > 0
        ...
        deltaD1 = ceil(deltaW1 / local_w_R);
...
fprintf('火箭故障总延误: %.1f 年\n', mean(results.rocket_time - results.rocket_trips * local_t_R));
fprintf('\n');
...
fprintf(fid, '火箭与太空电梯运输方案模拟报告 v3.1\n');
fprintf(fid, '生成时间: %s\n\n', datetime('now'));
fprintf(fid, '一、模拟概况\n');
fprintf(fid, '===========\n');
fprintf(fid, '模拟次数: %d\n', num_simulations);
